...
output.innerHTML =
	"Play with me! Click the button!" + "<br><br>" + output.innerHTML;
...
for (var i = 0; i < modals.length; i++) {
	...
	newGameButton.addEventListener("click", function() {
		endGame();
		clearTable();
		params.userround = window.prompt(
			"How many rounds do you want to play? :-)"
		);

		if (
			params.userround == null ||
			params.userround == "" ||
			isNaN(params.userround) ||
			params.userround < 1
		) {
			newGame.insertAdjacentHTML(
				"afterend",
				"<br><br>Hey, dont you want to play? Give me number!"
			);
		} else if (params.userround > 0) {
			newGameButton.disabled = true;
			button1.disabled = false;
			button2.disabled = false;
			button3.disabled = false;
			roundLeft.innerHTML =
				"<br><br>Rounds to play: " + (params.userround - params.round);
		}
	});
	...
	var game = function(user, kompchoice) {
		...
		if (user == kompchoice) {
			...
			(user == "stone" && kompchoice == "scissors") ||
			(user == "paper" && kompchoice == "stone") ||
			(user == "scissors" && kompchoice == "paper")
		) {
			params.userresult++;
			params.round++;
			scoresView.insertAdjacentHTML(
			...
			(kompchoice == "stone" && user == "scissors") ||
			(kompchoice == "paper" && user == "stone") ||
			(kompchoice == "scissors" && user == "paper")
		) {
			params.kompresult++;
			params.round++;
			scoresView.insertAdjacentHTML(
		...
		var resultsToTable = function() {
			...
			finalResultCell.insertAdjacentHTML(
				"beforeend",
				params.userresult + " - " + params.kompresult
			);
		}; // tu powinien byc srednik
		...
		var resultToModal = function(modalResult) {
			...
			modalHeader.innerHTML =
				"<header>" + modalResult + "</header>" + "<p> Play again! </p>";
		};
	...
	for (var i = 0; i < gameButtons.length; i++) {
		gameButtons[i].addEventListener("click", function() {
			...
			roundLeft.innerHTML =
				"<br><br>Rounds left: " + (params.userround - params.round);
			results.innerHTML =
				"Scores" +
				"<br><br>" +
				"Computer: " +
